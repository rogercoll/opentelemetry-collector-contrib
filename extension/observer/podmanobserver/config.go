...
package podmanobserver // import "github.com/open-telemetry/opentelemetry-collector-contrib/extension/observer/podmanobserver"
...
// Config defines configuration for podman observer
type Config struct {

	// The URL of the podman server.  Default is "unix:///var/run/podman.sock"
	Endpoint string `mapstructure:"endpoint"`

	// The maximum amount of time to wait for podman API responses.  Default is 5s
	Timeout time.Duration `mapstructure:"timeout"`

	// If true, the "Config.Hostname" field (if present) of the podman
	...
	// accessible to an instance of the agent running outside of the podman network stack.
	...
	// to an instance of the agent running outside of the podman network stack.
	...
	// through the podman event listener example: cache_sync_interval: "20m"
	// Default: "60m"
	CacheSyncInterval time.Duration `mapstructure:"cache_sync_interval"`

	// Podman client API version. Default is 1.22
	PodmanAPIVersion float64 `mapstructure:"api_version"`
}
...
func (config Config) Validate() error {
	...
	if config.PodmanAPIVersion < minimalRequiredPodmanAPIVersion {
		return fmt.Errorf("api_version must be at least %v", minimalRequiredPodmanAPIVersion)
	}
	if config.Timeout == 0 {
